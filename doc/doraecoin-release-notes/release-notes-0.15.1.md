Doraecoin Core version *0.15.1* is now available from:

  <https://download.doraecoin.org/doraecoin-0.15.1/>
...
A number of changes to the way Doraecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 doraes per kB (0.00001 DRC). This is to prepare for dropping the minimum transaction fee to 0.1 doraes per kB (0.0001 DRC) in the next release.
