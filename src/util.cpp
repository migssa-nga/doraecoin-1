...
const char * const BITCOIN_CONF_FILENAME = "doraecoin.conf";
const char * const BITCOIN_PID_FILENAME = "doraecoin.pid";
...
#else
    const char* pszModule = "doraecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Doraecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Doraecoin";
#else
    // Unix
    return pathRet / ".doraecoin";
#endif
#endif
}
