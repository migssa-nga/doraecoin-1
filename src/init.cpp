...
void Shutdown()
{
    ...
    RenameThread("doraecoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/doraecoin-project/doraecoin>";
    const std::string URL_WEBSITE = "<https://doraecoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("doraecoin-loadblk");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if doraecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if doraecoin is started while in a temporary directory.\n",
