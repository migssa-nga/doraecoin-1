...
LITECOIND=${BITCOIND:-$BINDIR/doraecoind}
LITECOINCLI=${BITCOINCLI:-$BINDIR/doraecoin-cli}
LITECOINTX=${BITCOINTX:-$BINDIR/doraecoin-tx}
LITECOINQT=${BITCOINQT:-$BINDIR/qt/doraecoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
DRCVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
